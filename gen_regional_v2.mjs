...
<style>
  ...
  .hero-eyebrow {
    ...
    color: var(--gold);
    /*
      A crisp, tight shadow -- not the blurry wide-radius kind removed
      elsewhere this session. This is thin, widely-tracked monospace text
      sitting directly on a photo (no background box, unlike hero-sub),
      and different hero photos put very different things behind it --
      some give gold enough contrast on its own, but several have pale
      sky or white cloud right where this label sits, where gold nearly
      vanishes. A plain white swap doesn't fix it either -- white washes
      out against white clouds just as badly. A tight dark rim around the
      letters reads clearly against any of them without changing the
      label's color or looking hazy up close.
    */
